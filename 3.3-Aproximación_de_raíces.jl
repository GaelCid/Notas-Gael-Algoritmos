...
Para $x_1$ a la interseccion de $g(x)$ con el eje x, tenemos: $x_1 = x_0- \frac{f(x_0)}{f'(x_0)}$

Asi en $x_{i+1}$ tenemos $x_1 = x_i- \frac{f(x_i)}{f'(x_i)}$
"""
...
begin
	function newton(f,f´,x,n)
	a_raiz= x -(f(x)/f´(x))

	for i in 1:n
		a_raiz = a_raiz - (f(a_raiz)/f´(a_raiz))
	end
		print(a_raiz)
	end
end

# ╔═╡ 2082054f-1064-44a7-a632-73eed9383628
begin
	g(x)= x^3-x-1
	g´(x)= 3*x^2
end

# ╔═╡ eadfe82f-52a1-4ff9-8cbb-d5819e243a1a
newton(g,g´,2,3)
...
# Tu código (comentado) va aquí :D
begin
	function NewtonR(f,f´,x,n)
	a_raiz= x -(f(x)/f´(x))
		n-=1
		if n!=0
		NewtonR(f,f´,a_raiz,n)
		end
		print(" ")
		print(a_raiz)
	end
...
# ╔═╡ 3f7c236f-65f0-465f-93ae-96ca52615259
NewtonR(g,g´,2,3)
...
# Tu código (comentado) va aquí :D
begin
	function nwetonD(f,f´,x,epsilon)
	if epsilon <= 0
...
# ╔═╡ a7575877-ea5d-4484-b9a4-16fa5a7489ac
nwetonD(g,g´,2, 4)
...
# Tu código (comentado) va aquí :D
DNumerica(f,x,h) = h * (f(x+(1/h))-f(x))


# ╔═╡ c3f04a76-37f8-41d0-a2a8-f0dee83c9c24
begin
	x=10
	h=10000
end

# ╔═╡ 6f11edbb-de16-48d4-8fba-17fdb323dadb
DNumerica(g,x,h)
...
# Tu código (comentado) va aquí :D
newtondn(f´,x,h)= x - (f´(x))/DNumerica(f´,x,h)

# ╔═╡ d18a6327-8d1a-46f2-88c2-990ea9dba0d6
newtondn(g´,1,3)
