...
# ╔═╡ 844c5d71-3f15-4d7f-a9e6-ee5a158662cf
tamañoDePaso(0,10,100)
...
function arregloUniforme(t0, tf, N, h)
    ...
    paso = (tf - t0) / N
    if paso != h
        println("Advertencia: El tamaño de paso h no coincide con el valor calculado a partir de N.")
    end
...
# ╔═╡ e61d0453-41bf-44f9-b17d-14f40491f745
arregloUniforme(3,3,-1,3)
...
# ╔═╡ 62a29dce-5bac-494e-af90-1871d811c386
# Tu código (comentado) va aquí :) 
function paso_euler(ti, xti, g, h)
    # Calcula la aproximación de x(ti + h) utilizando el método de Euler
    return xti + h * g(ti, xti)
end

# ╔═╡ f5f71618-ef60-4c27-ae3b-c548759eda4e
# Define la función g
g(ti, xi) = xi - ti^2 + 1

# ╔═╡ 3e5bab44-3e7c-449d-aaee-ce5fbcbfe530
# Calcula una aproximación de x(1.5) utilizando un tamaño de paso de h = 0.1
x1 = 0.5

# ╔═╡ 8a26a689-8d43-4844-9a8f-578a36b8f31c
h = 0.1

# ╔═╡ 34ea0b1d-ceb2-4635-9512-29ac77586f45
x2 = paso_euler(1.0, x1, g, h)

# ╔═╡ 0495994d-ae54-4235-af20-98913578a7a2
# Imprime el resultado
println("Aproximación de x(1.5) =", x2)
...
function euler(g, xt0, t)
    N = length(t) # Obtenemos la longitud del arreglo de tiempos
    x = zeros(N) # Creamos un arreglo para almacenar las aproximaciones de x(t_i)
    x[1] = xt0 # Asignamos la condición inicial xt0 al primer elemento del arreglo x

    for i in 1:N-1
        h = t[i+1] - t[i] # Calculamos el tamaño de paso h
        x[i+1] = paso_euler(t[i], x[i], g, h) # Calculamos la siguiente aproximación de x(t_i+1) usando el método de Euler
    end
...
# ╔═╡ 09c01386-5324-499a-a7ca-355afe46524e
euler(16,6,111)
...
#x(t) es el valor conocido de la función en un tiempo dado t.
#h es el paso de tiempo utilizado en la discretización.
#f(t) es la función que describe la derivada de la función en términos del tiempo.
