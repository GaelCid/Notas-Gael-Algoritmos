...
begin
    l = 0.06         # ¡Cambia el valor de l y corre la celda,
...
# ╔═╡ 2303a36b-3296-4c31-b990-40c0aa30109e
begin
...
# ╔═╡ da4d610b-dd8f-419b-9d56-844b62a7528e
begin
...
# ╔═╡ 7749fab0-b5e3-467b-8b29-888c28e8a6ad
@bind d Slider(0.002:0.05:5, default=0.5)

# ╔═╡ 061dc1a5-b753-43ca-871c-aa34c873c6ca
begin
    plot(sin,-π:d:π, title = "Funciones Trigonométricas" , label = "sin(x)" , color = "purple")
	plot!(cos,-π:d:π, color = "green" , label = "cos(x)")
end
...
#       l1,l2,l3   l1,l2,l3  desplazamiento   l1,l2,l3   l1,l2,l3
...
quiver((-2.5:0.25:2.5)', (-2.5:0.25:2.5) , quiver=g, color=:green)
...
contourf(R,R,h, ylabel = "y")
#contourf(R,R,h)=contour(R,R,h,fill=true)
...
scatter(0:10,0:10,0:10, label = "puntos 1", color = "black")
...
k = surface(R,R,h, color = :gist_rainbow)
...
begin
	...
	plot(f.( range(0,2π, step = 0.1) .- t/B) ,legend = false , title = L"A\sin(Bx-t)+C")
...
# ╔═╡ d39986aa-0aa4-4621-8f12-aa2cc58ea79d
begin
	h2 = 1000
	r = 10
	t = 100
	...
	fx(x) = vx * x
	fy(x) = (h2 + (vy) * x - ((9.8 * r * x^2) /2))
	p1(x) = (fx(x), fy(x))

	tiro_parabolico = @animate for t in 0:0.05:5.5
		plot(p1.(range(-(10),0,step = 0.1).+ t), legend = false, xlabel = "Distancia", ylabel = "Altura", title = "Tiro parabólico", ls=:dot, lw=5, color = "purple")
	scatter!([fx(t)],[fy(t)])
	end
	gif(tiro_parabolico, "Tiro-Parabólico.gif", fps = 30)
	end
...
begin 
	function tiro_prabolico(h1,r,t,θ)
	vx = r * cos(θ)
	vy = r * sin(θ)
	...
	tiro_parabolico = @animate for t in 0:0.05:5.5
		plot(p.(range(-(10),0,step = 0.1).+ t), legend = false, xlabel = "Distancia", ylabel = "Altura", title = "Tiro parabólico", ls=:dot, lw=10, color = "blue")
	scatter!([fx(t)],[fy(t)])
	end
	gif(tiro_parabolico, "Tiro-Parabólico.gif", fps = 30)
	end
...
# ╔═╡ 34733b1b-873c-4d8b-9e4b-361e88e6e16e
tiro_prabolico(10000,10,5,90)
...
x = 0:0.05:2pi
	...
	@gif for t in 0:0.1:2pi
	surface(x, y, h, legend = false , title = L"cos(x) + sin(y)", color = :CMRmap, size =(500,300), rotate=(30,30,30), camera=(8t,20) )
	end
end

# YANOQUIEROAGARRARLACOMPUTADORAPORMUCHOTIEMPONETAYAMEDUELEMIMUÑECAxd

# ╔═╡ fe4951f9-b781-48ac-8d86-715c7b9e39d1


# ╔═╡ 3d0f6ba7-17a9-4abc-a8ec-460d626a6a13
begin
	n = 300
	VALORX = range(0,10,length = n)
	VALORY = VALORX

@gif for i in range(0,2pi,length = n)
	no(x,y) = cos(x)+sin(y + 20cos(i))
	p = plot(VALORX,VALORY,no,st=:surface)
	default(legend = false, title = L"cos(x) + sin(y)" , color = :gist_rainbow, size =(500,300))
end
end
